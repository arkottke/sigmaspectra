...
bool MotionLibrary::interp(const QVector<double> &x, const QVector<double> &y,
    ...
    if (std::fabs(xi.first() - x.first()) > std::numeric_limits<double>::epsilon() && xi.first() < x.first()) {
        qCritical() << QString(
...
double MotionLibrary::factorial(const int n) {
    return exp(n * log(n) - n + (log(n * (1 + 4 * n * (1 + 2 * n)))) / 6 + log(M_PI) / 2);
}
...
double MotionLibrary::countTrials() {
    if (m_seedSize > m_suiteSize || m_suiteSize > m_motionCount) {
        ...
        int motionCount = (m_motionCount - m_disabledCount) / (m_combineComponents ? 2 : 1);
