...
MainWindow::MainWindow(QMainWindow *parent)
    ...
    connect(this, SIGNAL(disabledCountChanged(int)), m_motionLibrary,
            SLOT(setDisabledCount(int)));
    connect(m_motionLibrary, SIGNAL(motionCountChanged(int)), this,
            SLOT(updateMotionCount(int)));
...
void MainWindow::about() {
    QMessageBox::about(
            this, tr("About SigmaSpectra"),
            QString(tr("SigmaSpectra was written by Albert Kottke"
                               " and is released under the GNU General Public License (GPL)."
                               " Bug reports and feature requests can be made at the website."
                               "<br><br>"
                               "<a href='https://github.com/arkottke/sigmaspectra'>"
                               "https://github.com/arkottke/sigmaspectra"
                               "</a>"
                               "<br><br>"
                               "Albert Kottke <a "
                               "href='mailto:albert.kottke@gmail.com'>albert.kottke@gmail."
                               "com</a>"
                               "<br><br>"
                               "Version: %1"
                               "<br><br>"))
...
void MainWindow::removeRow() {
    ...
    m_tableView->model()->removeRows(selectedRows.first().row(),
                                     selectedRows.size());
...
void MainWindow::flagMotions() {
    if (m_motionLibrary->readMotions()) {
        ...
        int requested = 0;
        int disabled = 0;
        for (AbstractMotion *am : m_motionLibrary->motions()) {
            switch (am->flag()) {
        ...
        m_minRequestedCountSpinBox->setMaximum(
                qMin(requested, m_motionLibrary->suiteSize()));
    }
}
...
void MainWindow::createPage() {
    ...
    connect(m_dampingSpinBox, SIGNAL(valueChanged(double)), m_motionLibrary,
    ...
    connect(m_interpolateCheckBox, SIGNAL(toggled(bool)), m_motionLibrary,
            SLOT(setPeriodInterp(bool)));
    ...
    connect(m_periodSpacingComboBox, SIGNAL(currentIndexChanged(int)),
            m_motionLibrary, SLOT(setPeriodSpacing(int)));
    ...
    connect(m_periodMinSpinBox, SIGNAL(valueChanged(double)), m_motionLibrary,
            SLOT(setPeriodMin(double)));
    ...
    connect(m_periodMaxSpinBox, SIGNAL(valueChanged(double)), m_motionLibrary,
            SLOT(setPeriodMax(double)));
    fLayout->addRow(tr("Maximum:"), m_periodMaxSpinBox);

    m_periodCountSpinBox = new QSpinBox;
    m_periodCountSpinBox->setRange(50, 300);
    connect(m_periodCountSpinBox, SIGNAL(valueChanged(int)), m_motionLibrary,
            SLOT(setPeriodCount(int)));
    fLayout->addRow(tr("Number of Points:"), m_periodCountSpinBox);

    m_periodGroupBox = new QGroupBox(tr("Period Interpolation"));
    m_periodGroupBox->setEnabled(false);
    connect(m_interpolateCheckBox, SIGNAL(toggled(bool)), m_periodGroupBox,
    ...
    connect(m_pathLineEdit, SIGNAL(textEdited(QString)), m_motionLibrary,
            SLOT(setMotionPath(QString)));
    ...
    connect(m_suiteSizeSpinBox, SIGNAL(valueChanged(int)), this,
            SLOT(updateSuiteSize(int)));
    ...
    connect(m_seedSizeSpinBox, SIGNAL(valueChanged(int)), m_motionLibrary,
            SLOT(setSeedSize(int)));
    ...
    connect(m_suiteCountSpinBox, SIGNAL(valueChanged(int)), m_motionLibrary,
            SLOT(setSuiteCount(int)));
    ...
    connect(m_combinCheckBox, SIGNAL(toggled(bool)), m_motionLibrary,
            SLOT(setCombineComponents(bool)));

    m_stationCheckBox = new QCheckBox(tr("One component per recording station"));
    connect(m_combinCheckBox, SIGNAL(toggled(bool)), m_stationCheckBox,
            SLOT(setDisabled(bool)));
    connect(m_stationCheckBox, SIGNAL(toggled(bool)), m_motionLibrary,
            SLOT(setOneMotionPerStation(bool)));
    ...
    connect(this, SIGNAL(requiredCountChanged(int)), spinBox,
            SLOT(setValue(int)));
    ...
    spinBox->setButtonSymbols(QAbstractSpinBox::NoButtons);
    spinBox->setReadOnly(true);
    connect(this, SIGNAL(requestedCountChanged(int)), spinBox,
            SLOT(setValue(int)));
    ...
    spinBox->setButtonSymbols(QAbstractSpinBox::NoButtons);
    spinBox->setReadOnly(true);
    connect(this, SIGNAL(disabledCountChanged(int)), spinBox,
            SLOT(setValue(int)));
    ...
    connect(m_motionLibrary, SIGNAL(logText(QString)), m_textEdit,
            SLOT(append(QString)));
    layout->addWidget(m_textEdit, 0, 0, 1, 5);

    m_progressBar = new QProgressBar;
    m_progressBar->setRange(0, 100);
    connect(m_motionLibrary, SIGNAL(percentChanged(int)), m_progressBar,
            SLOT(setValue(int)));
    layout->addWidget(m_progressBar, 1, 0);

    m_etcLineEdit = new QLineEdit;
    m_etcLineEdit->setReadOnly(true);
    connect(m_motionLibrary, SIGNAL(timeChanged(QString)), m_etcLineEdit,
    ...
    m_cancelPushButton->setEnabled(false);
    connect(m_cancelPushButton, SIGNAL(clicked()), m_motionLibrary,
            SLOT(cancel()));
...
void MainWindow::loadValues() {
    ...
    m_periodSpacingComboBox->setCurrentIndex(
            (int) m_motionLibrary->periodSpacing());
