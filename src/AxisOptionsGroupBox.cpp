...
AxisOptionsGroupBox::AxisOptionsGroupBox( QwtPlot * plot, int axisId, const QString & title, QWidget * parent )
    : QGroupBox( title, parent ), m_plot(plot), m_axisId(axisId)
{
    ...
    // Load the values
    //
    if ( dynamic_cast<const QwtLinearScaleEngine*>(m_plot->axisScaleEngine(m_axisId)) )
