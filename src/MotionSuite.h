...
class MotionSuite : public QAbstractTableModel
{
    ...
    const QVector<double> & avgPlusStd() const;
    const QVector<double> & avgMinusStd() const;
    ...
    /*! Check if the suite is valid.
         */ 
    bool isValid(const int suiteSize, const int minRequestedCount, const QList<AbstractMotion*> & requiredMotions, const bool oneMotionPerStation ) const;
    ...
    QVector<double> m_avgPlusStd;

    //! Median minus one standard deviation
    QVector<double> m_avgMinusStd;
