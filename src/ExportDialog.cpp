...
ExportDialog::ExportDialog( const QList<MotionSuite*> & suites, const QString & motionPath, QWidget * parent, Qt::WindowFlags f)
...
void ExportDialog::tryAccept()
{
    ...
    if ( m_summaryCheckBox->isChecked() ) {
        QString destinationDir = ( type == MotionSuite::SHAKE2000Output ) ?
            QDir::currentPath() : m_destinationLineEdit->text();
