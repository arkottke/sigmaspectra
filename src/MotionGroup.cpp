...
bool MotionGroup::operator==( const MotionGroup & other ) const
{
    // This checks if the station and event are the same, and ignores the
    // orientation of the record.
    if ( event() == other.event() && station() == other.station() ) {
        return true;
    } else {
        return false;
    }
