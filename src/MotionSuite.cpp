...
MotionSuite::MotionSuite( const QVector<double>& period, const QVector<double>& targetLnSa, const QVector<double>& targetLnStd )
...
{
    QStringList list;
    list << "No Output" << "Strata" << "CSV" << "SHAKE2000";
...
const QVector<double> & MotionSuite::avgPlusStd() const
{
    return m_avgPlusStd;
}

const QVector<double> & MotionSuite::avgMinusStd() const
{
    return m_avgMinusStd;
}
                          ...
                          const QList<AbstractMotion*> & requiredMotions, const bool oneMotionPerStation) const
{
    ...
    int count = 0;
    foreach (const AbstractMotion * am, m_motions) {
    ...
    foreach (AbstractMotion * am, requiredMotions) {
...
void MotionSuite::computeScalars()
{
    ...
    m_avgPlusStd.resize(m_lnAvg.size());
    m_avgMinusStd.resize(m_lnAvg.size());
    
    for (int i = 0; i < m_lnAvg.size(); ++i) {
        m_avgSa[i] = exp(m_lnAvg.at(i));
        m_avgPlusStd[i] = exp(m_lnAvg.at(i) + m_lnStd.at(i));
        m_avgMinusStd[i] = exp(m_lnAvg.at(i) - m_lnStd.at(i));
    }
...
void MotionSuite::toText(QTextStream& os, MotionSuite::OutputType type)
{
    if ( type == SummaryOutput || type == CSVOutput ) {
        ...
        os << QString("Median RMSE,%1\nMedian Max Error (\%),%2\nStd RMSE,%3\nSigma Inf,%4\n")
                .arg(m_medianError).arg(m_medianMaxError).arg(m_stdevError).arg(m_sigmaInf);
